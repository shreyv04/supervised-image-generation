...
image_channels = 3
image_size = 64
num_classes = 10  # Labels for image generation (male, female, blond hairs, etc.)
batch_size = 64
num_epochs = 5
...
class Generator(nn.Module):
    def __init__(self, z_dim, image_channels, num_classes):
        ...
        self.model = nn.Sequential(
            nn.ConvTranspose2d(z_dim + num_classes, 256, kernel_size=4, stride=1, padding=0),
            nn.BatchNorm2d(256),
            nn.ReLU(),
...
fixed_labels = torch.tensor([i % num_classes for i in range(num_classes * 10)]).to(device)

# Transforms for the CelebA dataset
...
for epoch in range(num_epochs):
    for batch_idx, (real, labels) in enumerate(loader):
        real = real.to(device)
        labels = labels.to(device) % num_classes  # Restrict labels to range [0, num_classes-1]
    ...
    for i in range(20):
        row, col = divmod(i, 10)
        axes[row, col].imshow(fake_images[i].permute(1, 2, 0) * 0.5 + 0.5)  # Rescale to [0, 1]
        axes[row, col].set_title(f"Label: {fixed_labels[i].item()}", fontsize=8)
